...
class DAL(dict):
    ...
    def define_table(
        ...
        **args
        ...
        if tablename.upper() in self._adapter.KEYWORDS_COMMON:
            raise SyntaxError, 'invalid tablename "%s": is a reserved sql keyword' % tablename
        if tablename.upper() in self._adapter.KEYWORDS_ALL:
...
class Table(dict):
    ...
    def __init__(
        ...
        **args
        ...
        for field in fields:
            if field.name.upper() in self._db._adapter.KEYWORDS_COMMON:
                raise SyntaxError, 'invalid fieldname "%s": is a reserved sql keyword' % field.name
            if field.name.upper() in self._db._adapter.KEYWORDS_ALL:
                logging.warn('fieldname "%s" is a reserved sql keyword. it might not work on all databases' % field.name)
