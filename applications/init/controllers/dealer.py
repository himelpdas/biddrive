			...
			if my_message_form_dealer.process().accepted:
				...
				scheduler.queue_task(
					...
					pargs=['email', your.email, response.render('email_alert_template.html', dict(
						...
						MESSAGE =  XML("A dealer for a <i>%s</i> picked <b>%s</b> sent you a message."%(car, 'you')),
						MESSAGE_TITLE = "You have a new message!",
						WHAT_NOW = "The dealer said:",
				...
				if my_message_form_buyer.process(formname="buyer_message_form_%s"%offer_id).accepted: #The hidden field called "_formname" is generated by web2py as a name for the form, but the name can be overridden. This field is necessary to allow pages that contain and process multiple forms. web2py distinguishes the different submitted forms by their names. http://goo.gl/gTct7C
					...
					scheduler.queue_task(
						...
						pargs=['email', your.email, response.render('email_alert_template.html', dict(
							APPNAME=APP_NAME,
							NAME = your.first_name.capitalize(), 
							MESSAGE =  XML("The buyer for a <i>%s</i> picked <b>%s</b> sent you a message."%(car, 'you')),
							MESSAGE_TITLE = "You have a new message!",
							WHAT_NOW = "The buyer said:",
		...
		if a_winning_offer and session.BROADCAST_WINNER_ALERT: 
			scheduler.queue_task(
				...
				pargs=['email', your.email, response.render('email_alert_template.html', dict(
					...
					MESSAGE_TITLE = "Buyer picked a winner!",
					WHAT_NOW = "Try again!" if not is_winning_offer else "Wait for the buyer's call!",
					INSTRUCTIONS = "Look out for new buyer requests and bid quickly! Having attention of the buyer early on goes a long way." if not is_winning_offer else "The buyer will call you soon via our automatic validation system within your business hours!",
					CLICK_HERE = "Go to auction",
					CLICK_HERE_URL = URL(args=request.args),
