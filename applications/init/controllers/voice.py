...
def handle_key_check():
	...
	if digit_pressed == '1':
		...
		if winner_code_exists:
			...
			resp.dial(twiml.Number(winning_dealer_phone_number, url = URL("screen_for_machine.xml", vars = auction_request_vehicle)).split('/')[-1],) #convert init/voice/screen_for_machine.xml?model=... into screen_for_machine.xml?model=...
