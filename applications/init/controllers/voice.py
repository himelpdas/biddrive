...
def screen_for_machine():
	...
	with resp.gather(numDigits=1, action=URL("screen_for_machine.xml", args=[winning_offer_id], scheme=True, host=True), method="POST") as g: #if he pressed something go to a new function.
		for each in range(3):
			g.say(message)
			g.pause(length=3)
