...
def screen_for_machine():
	...
	if set(['_color', '_year', '_make', '_model', '_id']).issubset(request.vars): #test is list values in list w/o strf #http://goo.gl/mxqfX1
		...
		with resp.gather(numDigits=1, action="screen_complete", method="POST") as g: #if he pressed something go to a new function. #action would be screen_for_machine.xml/screen_complete
			for each in range(3):
				g.say(message)
				g.pause(length=3)
