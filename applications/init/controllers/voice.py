	...
	if digit_pressed == '1':
		...
		if winner_code_exists:
			...
			if not is_open_now: #means they're closed
				...
				auction_request_vehicle = dict(_color = color_names[winning_offer.color], _year = auction_request.year, _id=auction_request.id) #trim = auction_request.trim_name)
